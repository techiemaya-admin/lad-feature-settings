...
const requireFeature = (featureKey) => {
  return async (req, res, next) => {
    try {
      ...
      if (!organizationId) {
        console.error(
          "[Feature Guard] No organization ID found in req.user:",
          req.user,
        );
        return res.status(401).json({
          ...
          message:
            "Unable to verify feature access without organization identification",
        });
      }

      console.log(
        `[Feature Guard] Checking feature "${featureKey}" for org: ${organizationId}, user: ${userId}`,
      );
      ...
      console.log(
        `[Feature Guard] Feature "${featureKey}" enabled: ${isEnabled}`,
      );
