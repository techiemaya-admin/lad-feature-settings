...
const requireCredits = (usageType, creditsRequired) => {
  return async (req, res, next) => {
    try {
      ...
      logger.error("[Credit Guard] Error processing credits", {
        error: error.message,
      });
...
async function getCreditBalance(tenantId) {
  ...
  const schema =
    process.env.POSTGRES_SCHEMA || process.env.DB_SCHEMA || "lad_dev";
...
async function deductCredits(
  ...
  try {
    ...
    try {
      ...
      logger.warn("[Credit Guard] Failed to log to billing_ledger", {
...
async function getCampaignCreditUsage(campaignId, tenantId) {
  ...
  try {
    ...
    logger.error("[Credit Guard] Error getting campaign credit usage", {
      campaignId,
      error: error.message,
    });
    return {
...
const trackUsage = (usageType) => {
  return async (req, res, next) => {
    try {
      ...
      logger.error("[Credit Guard] Error tracking usage", {
        error: error.message,
      });
...
async function refundCredits(
  ...
  try {
    ...
    logger.info("[Credit Guard] Credits refunded", {
      credits,
      tenantId,
      usageType,
      reason,
    ...
    await client.query("ROLLBACK");
    logger.error("[Credit Guard] Error refunding credits", {
      error: error.message,
    });
    throw error;
  } finally {
