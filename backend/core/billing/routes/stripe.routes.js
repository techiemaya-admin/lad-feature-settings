...
router.post("/create-credits-checkout", authenticateToken, async (req, res) => {
  try {
    ...
    const STRIPE_FIXED = 1.0; // AED 1.00 (or equivalent in transaction currency)
    ...
    const session = await stripeClient.checkout.sessions.create({
      ...
      success_url:
        successUrl ||
        `${process.env.FRONTEND_URL}/settings?tab=credits&payment=success`,
      cancel_url:
        cancelUrl ||
        `${process.env.FRONTEND_URL}/settings?tab=credits&payment=cancelled`,
  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (!tenantId || !credits) {
          logger.error("[Stripe] Missing required metadata in webhook", {
            session,
          });
