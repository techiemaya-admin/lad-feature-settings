  ...
  try {
    ...
    if (chargeImmediately) {
      ...
      res.json({
        ...
        usageEvent,
        message:
          "Usage event created. Call POST /api/billing/charge/:usageEventId to charge.",
      });
    }
  } catch (error) {
    console.error("[Billing API] Error charging:", error);
    res
      .status(error.message.includes("Insufficient balance") ? 402 : 500)
      .json({
        success: false,
        error: error.message,
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      const wallet = await billingService.getWalletBalance(req.tenantId);
      const legacyBalance = await billingService.getLegacyCreditBalance(
        req.tenantId,
      );
      const useLegacyBalance = wallet.currentBalance <= 0 && legacyBalance > 0;
      const effectiveBalance = useLegacyBalance
        ? legacyBalance
      ...
      let monthlyUsage = usage.reduce(
        (sum, event) => sum + parseFloat(event.total_cost),
        0,
      );
      if (monthlyUsage === 0) {
        const legacyMonthlyTx =
          await billingService.listLegacyCreditTransactions({
            tenantId: req.tenantId,
            fromDate: firstDayOfMonth,
            toDate: now,
            limit: 5000,
