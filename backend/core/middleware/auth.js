...
const authenticateToken = (req, res, next) => {
  ...
  if (
    req.path.includes("/execute-followup") ||
    req.path.includes("/run-daily")
  ) {
  ...
  try {
    ...
    if (!process.env.JWT_SECRET) {
      details =
        "JWT_SECRET not configured. Check Google Cloud Secret Manager and Cloud Run environment variables.";
    }
