...
function getTenantContext(req) {
  ...
  logger.debug("[getTenantContext] Called", {
    hasUser: !!req?.user,
  ...
  try {
    ...
    if (!tenant_id) {
      ...
      tenant_id =
        req?.user?.tenantId || req?.user?.organizationId || req?.user?.clientId;
    }

    // If still no tenant ID, this is an error condition
    if (!tenant_id) {
      const error = new Error(
        "Tenant context required but not found in request",
      );
    ...
    const errorWithCode = new Error(
      "Failed to determine tenant context: " + error.message,
    );
