...
function requireTenantId(
  ...
  if (req?.user?.organizationId) {
    logger.debug(
      "[TenantHelper] Using organizationId from req.user as tenant ID",
      {
  ...
  const error = new Error(
    `Tenant ID required for ${operationName} but not found in request`,
  );
...
function getTenantId(explicitTenantId = null, req = null) {
  try {
    ...
    logger.debug("[TenantHelper] Could not extract tenant ID", {
      error: error.message,
    });
    return null;
  }
